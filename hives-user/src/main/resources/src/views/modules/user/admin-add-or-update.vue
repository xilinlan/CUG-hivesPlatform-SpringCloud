<template>
  <el-dialog
    ...
    <el-form-item label="登陆名称" prop="loginName">
      <el-input v-model="dataForm.loginName" placeholder="登陆名称"></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/user/admin/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.loginName = data.admin.loginName
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/user/admin/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
